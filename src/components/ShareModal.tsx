    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-text/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-md bg-bg border border-line max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="px-7 pt-8 pb-5 border-b border-line text-center">
          <div className="font-serif italic text-gold text-xs tracking-[0.3em] mb-3">— share this journey —</div>
          <h2 className="font-serif text-text text-3xl font-light tracking-wide">Share</h2>
          <p className="font-serif-ja text-text text-sm mt-2 tracking-wider">旅を同行者に共有する</p>
          <p className="font-serif-ja text-text-sub text-xs mt-3 tracking-wider">{tripTitle}</p>
        </div>
        <div className="px-7 py-7">
          {stage === 'pin' && (
            <>
              <p className="font-serif-ja text-text text-sm leading-relaxed mb-6 tracking-wider">共有用の4桁PINを設定してください。<br />同行者にはURLとPINを別々の手段で送ります。</p>
              <div className="space-y-5">
                <div>
                  <label className="font-sans text-xs tracking-[0.3em] text-accent uppercase block mb-2">Share PIN</label>
                  <input type="password" inputMode="numeric" maxLength={4} value={pin1} onChange={(e) => setPin1(e.target.value.replace(/\D/g, ''))} placeholder="••••" className="w-full bg-bg-alt border border-line px-4 py-3 font-serif text-2xl text-center tracking-[0.5em] text-text focus:outline-none focus:border-accent" />
                </div>
                <div>
                  <label className="font-sans text-xs tracking-[0.3em] text-accent uppercase block mb-2">Confirm</label>
                  <input type="password" inputMode="numeric" maxLength={4} value={pin2} onChange={(e) => setPin2(e.target.value.replace(/\D/g, ''))} placeholder="••••" className="w-full bg-bg-alt border border-line px-4 py-3 font-serif text-2xl text-center tracking-[0.5em] text-text focus:outline-none focus:border-accent" />
                </div>
                {error && (<div className="font-serif-ja text-sm text-olive bg-bg-alt border border-line p-3">{error}</div>)}
              </div>
              <div className="mt-7 space-y-3">
                <button onClick={handleGenerate} disabled={busy || !pin1 || !pin2} className="w-full py-3 bg-text text-bg font-serif text-xs tracking-[0.45em] uppercase disabled:opacity-40 hover:bg-accent transition-colors">{busy ? '— generating —' : '— Generate —'}</button>
          ...
          {stage === 'result' && (
            <>
              ...
              <button onClick={handleCopy} className="w-full py-3 border border-accent text-text font-serif text-xs tracking-[0.45em] uppercase hover:bg-accent hover:text-bg transition-colors mb-3">{copied ? '— Copied! —' : '— Copy URL —'}</button>
              ...
              {error && (<div className="font-serif-ja text-sm text-olive bg-bg-alt border border-line p-3 mt-4">{error}</div>)}
            </>
          )}
        </div>
