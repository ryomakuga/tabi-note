...
## 2026-06-01 — F-09 拡張:静止画+動画の合成ムービー(進行中の目標)

### 🎯 これが今やりたいこと(チャットが変わってもブレないための記録)
**Make a Movie で、静止画(写真)と動画クリップの両方を選んで、1本のムービーに合成できるようにする。**
- ステップ1の写真選択で、写真も動画も一緒に選べる(順番も保持)
- 動画が流れる間は BGM を小さくして、動画の中の声・音が聞こえるようにする(音声ミキシング/ダッキング)
- 編集時、動画パートの BGM 音量を5段階で選べるようにする

### 現状(2026-05-31 時点で検証済み)
- ✅ 段階6-1:写真+動画クリップの連結を ffmpeg.wasm で実現できると確認(実験関数 `testConcatPhotosAndVideos`)
- ✅ 段階6-2:連結動画に BGM を乗せられると確認(実験関数 `testConcatWithMusic`)。ただし動画の音はまだ無視
- ⚠️ 実験関数はコンソールから呼んで検証しただけ。**本番の MovieMaker UI には未統合**
- ⚠️ HEVC(H.265)動画は ffmpeg.wasm でコケる(`Invalid NAL unit size`)。H.264 ならOK。iPhone の新しめ動画は HEVC が多いので要対応(弾く or 警告)

### 残り作業
- 段階6-3:音声ミキシング(動画パートで BGM を下げ動画の声を出す)— 未検証・最難関
- 段階6-4:編集UIで動画パートの BGM 音量を5段階選択
- 実験関数を本番 MovieMaker に統合(ステップ1で写真も動画も選べるUI)
- HEVC 動画の扱い(弾く/警告)

### 進め方メモ
- まず案A(写真+動画の連結を本番UIに組み込む、音声は当面BGMのみ)を確実に完成させ、
  そのあと段階6-3(動画の声+5段階音量)に進む方針

出発まで:あと 27 日

---

## 2026-06-01(続き)— 写真+動画の合成ムービーを本番UIに統合(完了)

### できたこと
- 写真+動画を撮影日時順に1本へ合成する本番関数 `makeMixedMovie` を ffmpegTest.ts に追加(HEVC等で失敗した動画はスキップして続行)
- Make a Movie の写真選択グリッドで動画もサムネ表示(PhotoThumb 動画対応、▶マーク)
- generate を makeMixedMovie に接続 → 写真と動画を両方選んでムービー化できるようになった
- 動作確認OK:写真2枚+動画1本で28秒のムービー、BGM付き、スキップ0

### 並び順の決定
- 撮影日時順(takenAt)で自動。写真も動画も混ぜてこの順に並べる

### git
- 85551b6(generate接続)/ a029741(サムネ)/ ba2f8be(makeMixedMovie追加)/ 449d27f(ログ)= push済み

### 次の残り作業
- 段階6-3:動画パートでBGMを下げ、動画の中の声を出す(音声ミキシング)— 未着手・最難関
- 段階6-4:編集UIで動画パートのBGM音量を5段階で選べるように
...
## 2026-06-11 F-09 ムービーにズーム追加・表示時間延長・動画キャプション対応
### 目標
ムービーをより映画的に。写真にゆっくりズーム(Ken Burns)を効かせ、1枚あたりの
表示時間を延ばし、さらに **動画素材にもキャプションを乗せ、動画を切らず最後まで再生** する。
### 完了(コミット済み)
- **コミット `a0004c1`**: 写真にKen Burnsズーム追加、表示時間を2秒→4.5秒に延長。
  - `src/movie/ffmpegTest.ts` 写真側の `baseVf` に `zoompan`(z=1〜1.08, 中央寄せ)を追加。
  - `src/movie/MovieMaker.tsx` 164行: `makeMixedMovieWithDucking` 第3引数(秒数)を `2`→`4.5`。
    ※ ffmpegTest.ts側のデフォルトを変えても効かず、**呼び出し側の引数が本当に効く場所**だった。
- **コミット `b3b4136`**: 動画にもキャプション対応。
  - `ffmpegTest.ts` 動画分岐(779行〜)を、キャプションPNGがある時は overlay する版に。
    キャプションPNGを `-loop 1` で読み、`overlay=...:shortest=1` で **動画の尺に合わせて** 合成。
    `-shortest` 単独で無音0.1秒に切られていたのを修正し、**動画が最後まで流れる** ように。
  - `MovieMaker.tsx` 330行: 動画を除外していた `if (video) return null` を削除。
    動画にもキャプション入力欄を表示。サムネは `<video src=...#t=0.1>` で **動画の冒頭フレーム** を表示。
### 動作確認済み
- 写真: ズーム + キャプション(日本語/英字) + 日付 + スポット名、すべて表示OK。
- 動画: キャプションが乗り、切れずに最後まで再生。入力欄に動画サムネ表示OK。
- 音楽ダッキング・表紙タイトルも併用して正常。
### 次回やること(やり残し)
1. テロップの「ふわっと出る」フェード演出。別レイヤーへの `fade=alpha` / `geq` は
   loop静止画では効かないことが判明(何度試しても消える)。
   → 次は **キャプションを写真に焼き込んでから動画化** する焼き込み方式を、設計してから挑む。
2. 実機iPhoneでのPWA通しテスト(出発6/28前)。
### 注意・メモ
- `fade=t=in/out:alpha=1`、`geq` でのアルファ時間制御、合成後映像への通常fade、
  いずれも overlay レイヤー方式では文字が消える。FFmpeg.wasm + loop静止画 特有の壁。
- 安全地点: `a0004c1`(ズーム+表示時間)、`b3b4136`(動画キャプション)。戻る時はこのSHAへ。

## 2026-06-11(続き) キャプション「文字だけふわっと出現」完成
### 完了(コミット済み)
- `173b801`: 各カットに黒フェードイン/アウト(写真もキャプションも一緒にふわっと)。
- `ede6095`: 本命「文字だけふわっと出現」。写真・動画とも、同じ素材の【文字なし版】→【文字あり版】を
  xfadeで繋ぐことで、写真は動かさず文字だけが浮かび上がる演出を実現。新関数 makeCaptionXfadeSeg 追加。
- `ccdb381`: 文字を大きく(jp 44→56px / en 22→26px)、浮かび上がりをゆっくり(XF 1.2→2.4秒)に調整。
### 重要な学び
- 昨日まで全滅だった「文字のフェード」は、別レイヤーのアルファ(fade=alpha/geq)では
  FFmpeg.wasm+loop静止画で必ず消える。→ 解決策は xfade:「文字なし版」と「文字あり版」の
  2本を作って溶かす。アルファを一切使わないので地雷を踏まない。これが正解だった。
- トランジションは fade が安全。dissolve は映像全体が砂嵐ノイズになるため不採用。
- 動画側は acrossfade で音声も繋ぐ。
### 次回やること
1. 「文字がだんだん大きくなりながら出る」演出(キャプションPNG側だけズーム)※やるなら
2. 実機iPhoneでのPWA通しテスト(出発6/28前)
### 安全地点
